...
// TestInstallScriptSimulatedFlow simulates the exact flow of:
//   curl -fsSL .../install.sh | sh
//   curl -fsSL .../install.sh | bash
//
// The script runs: exec kilo-docker install "$@"
// With piped stdin from curl, autoConfirm must be true.
func TestInstallScriptSimulatedFlow(t *testing.T) {
	shells := []string{"sh", "bash"}
	...
	for _, shell := range shells {
		t.Run(shell, func(t *testing.T) {
			...
			// Simulate piped stdin (curl | sh or curl | bash)
			r, w, err := os.Pipe()
			if err != nil {
				t.Fatalf("failed to create pipe: %v", err)
			}
			...
			// Simulate: kilo-docker install (no -y, as install.sh does)
			os.Args = []string{"kilo-docker", "install"}
			...
			if !autoConfirm {
				t.Fatal("autoConfirm must be true for piped install — otherwise Replace it? prompt hangs or aborts")
			}

			// Simulate the handleInstall prompt
			result := promptConfirm("Replace it? [y/N]: ")
			if !result {
				t.Error("promptConfirm returned false — install would abort")
			}
		})
	}
}
